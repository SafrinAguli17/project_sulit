...
class Daftar_Judul extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $nama['nim_'] = $data['user']['nomor'];
            $nama['nama_'] = $data['user']['nama'];
    ...
    public function upload_draf()
    {
        ...
        if (!$this->upload->do_upload('draf')) {
            ...
            $nama['nama_user'] = $data['user']['nama'];
            $this->template->display('mahasiswa/fitur_1', $data + $nama + $error);
        } else {
            $this->form_validation->set_rules('nama', 'Nama', 'required|trim', [
                'required' => 'Nama Harus di Isi'
            ]);
            ...
            if ($this->form_validation->run() == false) {
                ...
                $kirim = [
                    ...
                    'hp' => ($this->input->post('hp')),
                    'judul_skripsi' => ($this->input->post('judul')),
                    'draf' => $upload_data['file_name'] . ('||') . $this->input->post('nim'),
                    'time' => time()
                ];
                $this->db->insert('list_pendaftar', $kirim);
